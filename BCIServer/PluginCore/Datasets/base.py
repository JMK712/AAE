...
class BaseDataset(Dataset):
    ...
    transform : callable | None
        On-the-fly transform applied to the Tutorials before it is returned.
    """
...
class WindowsDataset(BaseDataset):
    ...
    Parameters
    ----------
    windows : mne.Epochs | mne.EpochsArray
...
class BaseConcatDataset(ConcatDataset):
    ...
    def split(self, by=None, property=None, split_ids=None):
        ...
        if property is not None or split_ids is not None:
            warnings.warn("Keyword arguments `property` and `split_ids` "
                          "are new_m and will be removed in the future. "
                          "Use `by` instead.")
    ...
    def save(self, path, overwrite=False):
        ...
        overwrite: bool
            Whether to delete developing files (.json, .fif, -epo.fif) in specified directory
            prior to saving.
        """
...
class SubjectIDDataset(WindowsDataset):
    def __init__(self,windows, description, transform=None):
        super(SubjectIDDataset, self).__init__(windows, description, transform)
    ...
    def __getitem__(self, index):
        ...
        # otherwise get single 2d-tensor...
        crop_inds = self.crop_inds[index].tolist()
        subject_id = self.description['subject']
        return X, y, subject_id, crop_inds